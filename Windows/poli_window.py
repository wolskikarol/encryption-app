...
from Algorithms.poli import poli_encrypt, poli_decrypt

class PoliWindow(tk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller

        label = ttk.Label(self, text="Szyfr polialfabetyczny")
    ...
    def encrypt_text(self):
        ...
        key = self.key.get()
        encrypted_text = poli_encrypt(text, key)
    ...
    def decrypt_text(self):
        text = self.input_text.get("1.0", tk.END).strip()
        key = self.key.get()
        encrypted_text = poli_decrypt(text, key)
        self.result_text.delete("1.0", tk.END)
        self.result_text.insert(tk.END, encrypted_text)